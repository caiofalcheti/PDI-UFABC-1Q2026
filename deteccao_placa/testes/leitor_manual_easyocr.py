...
print("Inicializando EasyOCR... (o download do modelo pode levar alguns segundos na primeira execução)")
...
def processar_imagem(img):
    """Pré-processamento para realçar a placa."""
...
def detectar_placa(img_original, img_blackhat):
    """Localiza a placa e retorna a ROI."""
...
def refinar_caractere(roi_bin):
    """Refina um caractere individual para OCR."""
    ...
    roi = 255 - roi  # texto preto, fundo branco
    roi = cv2.copyMakeBorder(roi, 15, 15, 15, 15, cv2.BORDER_CONSTANT, value=255)
    return roi

def isolar_caracteres_duas_linhas(placa, exibir_debug=False):
    """
    Segmenta os caracteres da placa e retorna:
    - chars_sup, chars_inf (listas de imagens de caracteres)
    - placa_bin_inv (imagem binária da placa)
    - boxes (todos os bounding boxes válidos)
    - placa_vis (cópia da placa com retângulos desenhados)
    """
    ...
    placa_bin_inv = bin_placa.copy()  # para exibição
    ...
    # Cria imagem de depuração com caixas desenhadas
    placa_vis = placa.copy()
    for (x, y, w, h) in linha_sup:
        cv2.rectangle(placa_vis, (x, y), (x+w, y+h), (0, 255, 0), 1)  # verde = superior
    for (x, y, w, h) in linha_inf:
        cv2.rectangle(placa_vis, (x, y), (x+w, y+h), (255, 0, 0), 1)  # azul = inferior
...
def exibir_caracteres_isolados(chars_sup, chars_inf):
    """
    Cria uma imagem com todos os caracteres isolados lado a lado (para depuração).
    Retorna a imagem ou None se não houver caracteres.
    """
...
def ocr_linha_easyocr(chars):
    """Realiza OCR em uma linha montada de caracteres usando EasyOCR."""
    if not chars:
        return ""
    altura = 100
    espaco = 12
...
def ocr_placa_inteira(placa):
    """Fallback: OCR direto na placa inteira (para placas de 1 linha)."""
    if placa is None:
        return ""
    gray = cv2.cvtColor(placa, cv2.COLOR_BGR2GRAY)
...
def processar_frame_completo(frame):
    """
    Processa o frame e retorna:
    - texto da placa
    - placa (ROI)
    - placa_bin_inv
    - placa_vis (com caixas)
    - painel_caracteres
    """
    ...
    # Fallback para 1 linha (carro) se a segmentação produziu poucos caracteres
...
# =============================================================================
# PROGRAMA PRINCIPAL (CAPTURA MANUAL COM 'g')
# =============================================================================
if __name__ == '__main__':
    ...
    while True:
        ...
        cv2.imshow("Webcam - Pressione 'g' para processar", frame)
        key = cv2.waitKey(1) & 0xFF

        if key == ord('g'):
            print("Processando frame...")
            inicio = time.time()
            texto, placa, bin_img, placa_vis, painel = processar_frame_completo(frame.copy())
            tempo = time.time() - inicio

            if texto:
                print(f"Placa OCR: {texto}  (tempo: {tempo:.2f}s)")
            else:
                print("Nenhuma placa detectada ou texto vazio.")

            # Exibe as janelas de depuração (se disponíveis)
            ...
            if painel is not None:
                ...
                print("Nenhum caractere isolado para exibir.")
