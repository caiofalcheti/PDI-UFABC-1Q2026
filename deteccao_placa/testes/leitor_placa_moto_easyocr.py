...
# =============================================================================
INTERVALO_PROCESSAMENTO = 2.5       # segundos entre cada leitura
LARGURA_PROCESSAMENTO = 640         # redimensionamento para desempenho
...
# Inicializa o leitor EasyOCR (idioma inglês, sem GPU)
print("Inicializando EasyOCR... (isso pode levar alguns segundos na primeira execução)")
...
def ocr_linha_easyocr(chars):
    """Usa EasyOCR para reconhecer texto em uma linha montada de caracteres."""
    if not chars:
        return ""
    
    # Monta a imagem da linha (igual ao código Tesseract)
    altura = 100
    espaco = 12
    ...
    # EasyOCR: permite apenas caracteres alfanuméricos maiúsculos
    resultados = reader.readtext(img_linha, detail=0, paragraph=True)
    texto = resultados[0] if resultados else ""
    
    # Filtra apenas caracteres válidos (letras e números)
    texto_filtrado = ''.join(c for c in texto.upper() if c.isalnum())
...
def processar_frame_ocr(frame):
    ...
    texto_sup = ocr_linha_easyocr(sup)
    texto_inf = ocr_linha_easyocr(inf)
...
if __name__ == '__main__':
    ...
    print(f"\nLeitura automática a cada {INTERVALO_PROCESSAMENTO} segundos.")
